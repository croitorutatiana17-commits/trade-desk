...
target_dir = os.path.join(os.path.dirname(__file__), 'src/routes')
dollar = chr(36)  # $

files = {}

# Jobs detail page
files[f'jobs.{dollar}jobId.tsx'] = '''import { createFileRoute, Link } from '@tanstack/react-router'
    ...
    <div className="p-4 max-w-lg mx-auto space-y-4 pb-8 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-3">
        <div className="flex items-center justify-between">
          ...
          {nextStatus && <button onClick={() => setStatus(nextStatus)} className="bg-navy-900 text-white rounded-xl px-4 py-2 text-sm font-semibold hover:bg-navy-800">Mark {nextStatus === 'in_progress' ? 'In Progress' : 'Completed'}</button>}
          {(status === 'completed' || status === 'scheduled') && <Link to="/invoices/new" className="bg-amber-500 text-white rounded-xl px-4 py-2 text-sm font-semibold hover:bg-amber-400">{'📄'} Invoice</Link>}
        </div>
      ...
      <Link to="/invoices/new" className="w-full flex items-center justify-center gap-2 bg-amber-500 text-white rounded-2xl py-4 font-bold text-lg shadow-lg shadow-amber-500/25 hover:bg-amber-400 active:scale-[0.98] transition-all">
        {'📄'} Create Invoice
      </Link>
    </div>
  )
...
files[f'invoices.{dollar}invoiceId.tsx'] = '''import { createFileRoute, Link } from '@tanstack/react-router'
import { useState } from 'react'
...
print('All files:', sorted(os.listdir(target_dir)))